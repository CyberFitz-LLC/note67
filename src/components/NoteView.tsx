...
export function NoteView({
  ...
  const isTranscribing =
    useLiveTranscriptionStore((s) => s.isLiveTranscribing) &&
    isThisNoteRecording;
  ...
  // the count and the last row's id move together and are enough to notice.
  const {
    ...
    note.id,
    `${transcript.length}:${transcript[transcript.length - 1]?.id ?? 0}`
  );
  ...
  useEffect(() => {
    const loadSegments = async () => {
      ...
      if (migrated) {
        ...
        setPlayingAudioPath(
          (current) => current ?? first.mic_path ?? first.system_path
        );
      }
    };
